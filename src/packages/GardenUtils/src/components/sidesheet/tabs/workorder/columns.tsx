  ...
  return [
    ...
    },
    {
      ...
      Cell: (cellProps: CellProps<WorkOrderBase, number | null>): JSX.Element | null => {
        return <ProgressBar percentWidth={cellProps.value === null ? 0 : cellProps.value} />;
      },
      width: 100,
      Aggregated: () => null,
      aggregate: 'count',
        ...
        if (estimateHoursMax === -1) {
          const maxCount = Math.max(
            ...cellProps.cell.column.filteredRows.map((val) =>
              Number(val.original?.estimatedManHours === null ? 0 : val.original?.estimatedManHours)
            )
          );
    ...
    },
    {
      ...
      Cell: (cellProps: CellProps<WorkOrderBase, number | null>): JSX.Element => {
        if (remainingHoursMax === -1) {
          const maxCount = Math.max(
            ...cellProps.cell.column.filteredRows.map((val) =>
              Number(val.original?.remainingManHours === null ? 0 : val.original?.remainingManHours)
            )
          );
