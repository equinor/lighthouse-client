...
export function TimeChart<T>({ data, options: { title, timeChartOptions, colors, defaultTime } }: TimeBarChartProps<T>): JSX.Element {
    ...
    const options: ApexOptions = useMemo(() => ({
        ...
        colors: colors ? colors : [tokens.colors.infographic.substitute__blue_sky.hex, tokens.colors.infographic.primary__moss_green_100.hex,],
        xaxis: {
            type: 'category',
            tickPlacement: 'on',
            categories,
    ...
    return (
        <AutoSizer>
            {(
                ...
                <>
                    <TimeWrapper>
                        <TimeChip variant={getVariant("week")} onClick={() => setTime("week")}>Week</TimeChip>
                        <TimeChip variant={getVariant("month")} onClick={() => setTime("month")}>Month</TimeChip>
                        <TimeChip variant={getVariant("quarter")} onClick={() => setTime("quarter")}>Quarter</TimeChip>
                        <TimeChip variant={getVariant("year")} onClick={() => setTime("year")}>Year</TimeChip>
                    </TimeWrapper>
                    <Chart
                        options={options}
                        series={series}
