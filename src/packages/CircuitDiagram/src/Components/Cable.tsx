      ...
      <CableGroupWrapper>
        ...
        {!isFirstCable && isEditMode && cable.disconnected && (
          <DisconnectWrapper onMouseOver={onOpen} onMouseLeave={onClose}>
            <Checkbox
              ...
              onChange={async () => {
                const updatedCable = await reconnectCable(parentCircuitTagNo, cable.tagNo);
                if (updatedCable) {
              ...
              <CircuitDiagramPopover>
                {
                  <DisconnectedPopover>
                    <div>
                      Disconnected by{' '}
                      {cable.disconnectedBy?.firstName + ' ' + cable.disconnectedBy?.lastName}{' '}
        ...
        {!isFirstCable && isEditMode && !cable.disconnected && (
          <DisconnectWrapper onMouseOver={onOpen} onMouseLeave={onClose}>
            ...
            />
            {isOpen && (
              <CircuitDiagramPopover>{'Connected. Click to disconnect'}</CircuitDiagramPopover>
            )}
          </DisconnectWrapper>
        )}
          ...
          {status === CheckListStatus.OK ? ( //Even if data says cables are disconnected, if status is OK we draw the entire cable.
            ...
            >
              ...
              <div onMouseOver={onStatusOpen} onMouseLeave={onStatusClose}>
                ...
                {isCableStatusOpen && <CircuitDiagramPopover>{status}</CircuitDiagramPopover>}
              </div>
            </CableInfo>
          ) : isPulled && cable.terminatedFromDate === null && cable.terminatedToDate === null ? ( //pulled cable but not terminated in either side (two spirals)
            <CableWrapper>
              <CableNode>
                <DisconnectedStart>
                  <CableSpiralLeft isolated={cable.disconnected || circuitIsolated} />
                </DisconnectedStart>
                <CableInfo
                  borderBottom={borderBottom}
                  disconnectedCount={2}
                ...
                <DisconnectedEnd>
                  <CableSpiralRight isolated={cable.disconnected || circuitIsolated} />
                </DisconnectedEnd>
              </CableNode>
            </CableWrapper>
