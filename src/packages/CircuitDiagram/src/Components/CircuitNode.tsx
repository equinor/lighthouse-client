...
export const CircuitNode = ({
  ...
  function getNodeRender(
    ...
    switch (node?.eleSymbolCode) {
      case CircuitTypes.Circuit:
        return (
          <CircuitAndStarter
            ...
            eleNetwork={eleNetwork}
            pipetests={pipetests}
            cTestStatus={getCircuitTestStatus(CheckListStepTag.HtCTest, eleNetwork.checkLists)}
            isEditMode={isEditMode}
      ...
      case CircuitTypes.JunctionBox:
        return <JunctionBox value={node?.tagNo} status={nodeStatus} disconnected={disconnected} />;
  ...
  return (
    <>
      ...
      <CircuitDiagramVerticalRow key={cableNode !== undefined ? cableNode.tagNo : node?.tagNo}>
