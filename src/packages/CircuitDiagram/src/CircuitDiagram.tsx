...
export const CircuitDiagram = ({
      ...
      data
        ? Object.values(
            data?.reduce((acc, item) => {
              ...
              acc[circuitAndStarterTagNo[0]] = [...(acc[circuitAndStarterTagNo[0]] || []), item];
              return acc;
            }, {})
          )
        : [],
  ...
  if (circuitStarterTagNoString === '' || circuitStarterTagNoString === undefined) {
    return (
      <NoCircuitDiagramFound htCable={htCable !== undefined} releaseControl={pipetest === null} />
    );
  }
          ...
          <CircuitDiagramWrapper width={width}>
            <CircuitDiagramContainer width={width}>
              ...
              {switchboards?.map((eleNetworksForSwitchboard: EleNetwork[]) => {
                ...
                return (
                  <SwitchBoardContainer key={eleNetworksForSwitchboard[0].circuitAndStarterTagNo}>
                    <SwitchBoardBorderContainer>
                      <CircuitDiagramNodeGroupRow>
                        <CircuitDiagramNodeText
                          onClick={() =>
                        ...
                        <StatusCircle
                          statusColor={getCircuitDiagramCompletionStatusColor(switchboardStatus)}
                        />
                      </CircuitDiagramNodeGroupRow>
                      {eleNetworksForSwitchboard?.map((eleNetwork: EleNetwork) => {
                        const startNode = eleNetwork.circuits.find((x) => x.parentEleNetId === '');
                        return (
                          <CircuitDiagramRow key={eleNetwork.eleNetId}>
                            <CircuitNode
                              ...
                              disconnected={eleNetwork?.isolated === true}
                              comment={comment}
              ...
              {sidesheetType === 'rc' ? null : (
                <CircuitDiagramEditMode onMouseOver={onOpen} onMouseLeave={onClose}>
                  {!isEditMode && (
                    <Button onClick={() => setIsEditMode(true)}>Edit isolations</Button>
                  )}
                  {isEditMode && (
                    <Button
                      variant="outlined"
                  ...
                  {isOpen && (
                    <CircuitDiagramPopover cornerButton={true}>{buttonText}</CircuitDiagramPopover>
                  )}
                </CircuitDiagramEditMode>
              )}
            </CircuitDiagramContainer>
