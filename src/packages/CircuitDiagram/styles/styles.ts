...
export const CircuitDiagramHTText = styled.div<{ clickable?: boolean; highlight?: boolean }>`
  ...
  background-color: ${(p) => (p.highlight ? tokens.colors.interactive.primary__resting.hex : null)};
...
export const TestDotCircle = styled.div<{ color: string }>`
  ...
  color: ${(p) =>
    p.color === PipetestCompletionStatusColors.OK || p.color === PipetestCompletionStatusColors.PA
