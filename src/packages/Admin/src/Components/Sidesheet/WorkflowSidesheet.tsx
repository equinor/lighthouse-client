...
export function WorkflowSidesheet({ item, actions }: WorkflowSidesheetProps): JSX.Element {
  ...
  return (
    <Wrapper>
      <div>
        <SelectTemplateText>
          Select relevant workflow steps. The responsible role can stay empty, but must be filled in
          by the user when the template is selected in an app.
        </SelectTemplateText>
        <SelectTemplateText>
        ...
        {isLoading ? (
          ...
          <div>
            ...
            <SelectionRow>
              ...
              {steps.length === 0 && (
                <NewStepButton onClick={() => addStep(getNewWorkflowSteps(), WorkflowAdminAtomApi)}>
                  New flow
                </NewStepButton>
              )}
            </SelectionRow>
