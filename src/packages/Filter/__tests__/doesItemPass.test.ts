...
const valueFormatter: ValueFormatterFilter<ScopeChangeRequest> = {
  name: 'Change category',
...
const valueFormatterArray: ValueFormatterFilter<ScopeChangeRequest> = {
  name: 'WorkflowSteps',
  valueFormatter: () => [],
};
...
const valueFormatterArrayWithMultiple: ValueFormatterFilter<ScopeChangeRequest> = {
  name: 'WorkflowSteps',
  valueFormatter: ({ workflowSteps }) => workflowSteps && workflowSteps.map(({ name }) => name),
};

describe('Array items with multiple', () => {
  it('Should pass', () => {
    const result = doesItemPassFilter(
      arrayItemWithMultiple,
      [arrayFilterWithMultiple],
      [valueFormatterArrayWithMultiple]
    );
...
const valueFormatterArrayNull: ValueFormatterFilter<ScopeChangeRequest> = {
  name: 'Category',
  valueFormatter: ({ changeCategory }) => changeCategory.name,
};
...
describe('Null value to not pass', () => {
  it('Should not pass', () => {
    const result = doesItemPassFilter(
      nullValueTestData,
      [nullValuesTestFilterGroup],
      [valueFormatterArrayNull]
    );
    expect(result).toBeFalsy();
  });
});
