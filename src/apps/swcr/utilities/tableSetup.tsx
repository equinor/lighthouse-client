  ...
  },
  {
    ...
    Aggregated: ({ row }) => {
      const count = row.subRows.reduce((acc, curr) => {
        acc =
          curr.original.status !== 'Closed' && curr.original.status !== 'Closed - Rejected'
            ? acc + 1
            : acc;
        return acc;
      }, 0);
    ...
    Footer: (data) => {
      const count = data.data.reduce((acc, curr) => {
        acc = curr.status !== 'Closed' && curr.status !== 'Closed - Rejected' ? acc + 1 : acc;
        return acc;
      }, 0);
      return count;
    },
  ...
  },
  {
    ...
    Aggregated: (cell) => {
      const closed = cell.row.subRows.reduce((acc, curr) => {
        if (curr.original.status === 'Closed' || curr.original.status === 'Closed - Rejected') {
          acc++;
        }
