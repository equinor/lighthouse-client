...
export const ReleaseControlProcessForm = (): JSX.Element => {
  ...
  useQuery([value], {
    ...
    onSuccess: (data) => {
      ...
      updateAtom({
        workflowSteps:
          (data as any)[0].workflowStepTemplates.map((x: CreateReleaseControlStepModel) => {
            (x.description = ''), (x.criteriaTemplates = x.workflowStepCriteriaTemplates ?? []);
            return x;
          }) ?? [],
      });
    },
        ...
        <NavigationButton>
          {step === 'workflow' && (
            <Button variant="outlined" onClick={() => updateStep('scope')}>
              <Icon name={'chevron_left'} color={tokens.colors.interactive.primary__resting.rgba} />
        ...
        <ButtonContainer>
          {isLoading || isCreated ? (
            ...
            <>
              ...
              <Button disabled={!isValid} onClick={() => onMutate(true)} variant="outlined">
                Save
              </Button>
            </>
          )}
