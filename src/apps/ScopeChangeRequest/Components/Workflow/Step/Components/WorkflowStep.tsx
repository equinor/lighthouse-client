...
export function WorkflowStepContainer({ step }: WorkflowStepProps) {
    const { request } = useScopeChangeContext();

    const checkContributorAccess = () => canAddContributor({ requestId: request.id, stepId: step.id })
    const { data: isAllowedToAddContributor } = useQuery(`step/${step.id}`, checkContributorAccess, { refetchOnWindowFocus: false });
