...
export const tableConfig: TableOptions<ScopeChangeRequest> = {
  ...
  customColumns: [
    ...
    defineColumn({
      header: 'Disciplines',
      accessor: (s) =>
        s.disciplineGuesstimates.map(({ discipline: { procosysCode } }) => procosysCode).toString(),
      width: 90,
    }),
    defineColumn({
      header: 'Guess Mhrs',
...
function findNextToSign(sc: ScopeChangeRequest) {
  return (
    sc.currentWorkflowStep?.criterias.find((x) => x.signedAtUtc === null)?.valueDescription ?? null
  );
}
