        ...
        return (
          <StatusCustomCell
            ...
            cellAttributeFunction={() => ({
              style: {
                backgroundColor: cellProps.value !== '' ? statusColor : 'transparent',
              },
            })}
          />
        );
      ...
      Cell: (cellProps: CellProps<WorkOrder>) => {
        ...
        if (deref(estimateHoursMaxAtom) === -1) {
          const maxCount = Math.max(
            ...cellProps.cell.column.filteredRows.map((val) => Number(val.original?.estimatedHours))
          );
    ...
    },
    {
      ...
      Cell: (cellProps: CellProps<WorkOrder>) => {
        ...
        if (deref(actualHoursMaxAtom) === -1) {
          const maxCount = Math.max(
            ...cellProps.cell.column.filteredRows.map((val) => Number(val.original?.expendedHours))
          );
    ...
    },
    {
      ...
      Cell: (cellProps: CellProps<WorkOrder>) => {
        ...
        if (deref(remainingHoursMaxAtom) === -1) {
          const maxCount = Math.max(
            ...cellProps.cell.column.filteredRows.map((val) => Number(val.original?.remainingHours))
          );
