...
export const useGetWorkorders = (loopNo: string) => {
  ...
  const workorderRequestArgs = generateFamRequest(workorderColumnNames, 'Or', workorderExpressions);
  const {
    data: workorders,
    isLoading: isLoadingWorkorders,
    error: workorderError,
  } = useQuery(['workorder', loopNo], ({ signal }) => getWorkorders(workorderRequestArgs, signal));
