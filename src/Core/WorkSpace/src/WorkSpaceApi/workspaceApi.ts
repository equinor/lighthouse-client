...
>(options: WorkspaceOptions<T, SideSheetIds>): WorkSpaceApi<T> {
  ...
  const workspaceAPI: WorkSpaceApi<T> = {
    ...
    registerCustomContentView(
      ...
      updateState({
        viewComponent: viewComponent as React.FC<DataViewerProps<Record<PropertyKey, unknown>>>,
    ...
    registerTableOptions<T extends Record<PropertyKey, unknown>>(
      ...
      updateState({
        tableOptions: { onSelect, ...tableOptions } as TableOptions<Record<PropertyKey, unknown>>,
      });
