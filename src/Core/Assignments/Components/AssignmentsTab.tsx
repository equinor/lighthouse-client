...
export function AssignmentsTab(): JSX.Element {
  ...
  if (assignments.length === 0) {
    return <NoAssignments>You have no pending tasks</NoAssignments>;
  }
  ...
  return (
    <>
      <Assignments>
          ...
          <ActiveOrigins>
            {origins.map((applicationName) => (
              <Chip
                ...
                key={applicationName}
              >
                <div>{`${getCountForAppName(applicationName, assignments)} ${capitalize(
                  applicationName
                )}`}</div>
              </Chip>
            ))}
            ...
            items={[
              {
                ...
                icon: <Checkbox crossOrigin={undefined} checked={isGroupedBySource} readOnly />,
        ...
        <ScrollWrapper>
          {isGroupedBySource ? (
            <GroupedAssignments assignments={assignments} activeAssignments={activeAssignments} />
          ) : (
            <Wrapper>
              {assignments &&
                assignments
                  .filter(({ sourceSystem }) => activeAssignments.includes(sourceSystem.subSystem))
                  .map((assignment) => (
