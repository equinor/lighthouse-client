...
export const AssignmentCard = ({ assignment }: AssignmentCardProps): JSX.Element => {
  ...
  const handleClick = () => {
    if (assignment.type === 'External') {
      ...
      handleActionClick(assignment.sourceSystem.subSystem, assignment.sourceSystem.identifier);
    }
  };
  ...
  return (
    <Wrapper onClick={handleClick}>
      ...
      <TimeStamp>
        {assignment.created && DateTime.fromJSDate(new Date(assignment.created)).toRelative()}
      </TimeStamp>
    </Wrapper>
  );
};
